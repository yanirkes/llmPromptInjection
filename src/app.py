...
def main():
    ...
    try:
        ...
        if letter_topic_input:
            glove_vectors = KeyedVectors.load_word2vec_format(r"src\glove-wiki-gigaword-50.gz", binary=False)
            # map between the different models
            to_prompt = protectors_mapper.ProtectorRouter.map_prompt("letter_layer", letter_topic_input, glove_vectors)
